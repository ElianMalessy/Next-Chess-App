...
export default function FriendDialog({username}: {username: string}) {
  const [openDialog, setOpenDialog] = useState(false);
  const [alert, setAlert] = useState('');
  const {currentUser} = useAuthStore();
  const firstRender = useRef(true);
  useEffect(() => {
    if (!(currentUser?.displayName && username && firstRender.current) || currentUser?.displayName === username) return;
